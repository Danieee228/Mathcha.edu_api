...
export const LessonLearningPage = () => {
  ...
  const getEmbedUrl = (url) => {
    ...
    try {
      ...
      console.log("Lỗi parse URL video:", error);
    }
    // Nếu trong CSDL chỉ lưu mã ID ngắn (VD: abc123xyz), thì giữ nguyên logic cũ của bạn:
    return url.length < 20 ? `https://www.youtube.com/embed/${url}` : url;
  };
          ...
          <div className="bg-no-repeat bg-blue-50 border-4 border-black rounded-xl w-full h-120 py-3 flex flex-col items-center ">
            ...
            {lesson.video_url && (
              <iframe
                ...
                width="918"
                height="517"
                src={getEmbedUrl(lesson.video_url)} // GỌI HÀM Ở ĐÂY
                title="Video"
                frameBorder="0"
            ...
            <div className="">
              <button
                className={
                  "py-1 px-2 rounded-xl bg-blue-500 text-white border-2 border-black flex items-center justify-end gap-2"
                }
