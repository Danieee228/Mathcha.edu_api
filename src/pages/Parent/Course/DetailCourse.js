...
export default function DetailCourse() {
  ...
  const validateForm = () => {
    ...
    if (!formData.username) {
      ...
      formData.username.length > maxLength
    ) {
    ...
    if (!formData.password) {
      ...
      formData.password.length > maxLength
    ) {
  ...
  useEffect(() => {
    const fetchChaptersTopicsAndLessons = async () => {
      ...
      const chaptersWithTopicsAndLessons = await Promise.all(
        chaptersData.map(async (chapter) => {
          const topicsResponse = await api.get(
            `topic/chapter/${chapter.chapter_id}`,
          );
  ...
  const handlePurchase = async () => {
    if (!selectedStudentId)
      return toast.error("Vui lòng chọn học sinh để đăng ký!");

    try {
      // Nhịp 1: Tạo đăng ký khóa học
      const { data } = await api.post("/enrollment/create", {
        student_id: selectedStudentId,
        course_id: courseId,
      });

      // Nhịp 2: Thanh toán ảo lưu Database
      await api.post("/payment/fake-success", {
        user_id: user?.user_id || user?.id,
        enrollment_id:
          data?.enrollment_id || data?.data?.enrollment_id || data?.id,
        amount: String(course?.discount_price * 1000 * 100),
      });

      toast.success("Thanh toán thành công! Bé đã có thể bắt đầu học.");
    } catch (error) {
  ...
  const handleGetStudentId = (id) => setSelectedStudentId(id);
  // Create Student
          ...
          <div className="ml-10">
            ...
            <div className="text-base leading-7 text-gray-700">
              ...
              <div className="max-w-xl">
                <p className="mt-6">
                  <strong className={"mr-1"}> - Mô tả khóa học: </strong>{" "}
                  {course.description}.
                </p>
                <p className="my-5">
                        ...
                        >
                          ...
                          {topic.lessons &&
                            topic.lessons.map((lesson, lessonIndex) => (
                              <DisclosurePanel
                                key={lessonIndex}
                                className="flex items-center rounded-xl bg-black/5 justify-between p-3 my-3 ml-6 text-sm/5"
                              >
                                <div>
                            ...
                            <div className="flex items-center w-full">
                              ...
                              <div className="flex-1 flex justify-center">
                                <input
                                  ...
                                  onChange={() =>
                                    handleGetStudentId(student.student_id)
                                  }
                                  className="mr-2"
                                />
                              </div>
